...
@dp.message_handler(IsAdmin(), commands=['moderator'])
async def make_changes_command(m: types.Message):
...
async def cm_start(m: types.Message):
...
@dp.callback_query_handler(exam_callback.filter(action='delete'))
